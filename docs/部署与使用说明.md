...
> 0.3.0 Beta 更新：Windows x64 的 Zotero 9 companion add-on 已把自包含 Bridge 打包进单个 XPI。普通 Zotero 用户不再需要 Python、pip、项目源码或 launcher 注册；只需另行安装 Pi CLI。本文保留的 Python/Obsidian/MCP 流程用于源码开发和独立 Bridge 部署。
...
2. Obsidian 主控插件
- 运行在 Obsidian Desktop 内
- 负责生成 bridge 配置
- 负责启动、停止、重启 Python bridge 子进程
- 负责检查 `/health` 和 `/capabilities`
- 是面向日常同步工作流的主产品入口
...
职责：
- 作为 Obsidian Desktop 内的主控插件
- 管理 bridge 子进程生命周期
- 写入当前 vault 对应的 bridge 配置
- 提供 bridge 状态检查和日志入口
- 后续承载 Obsidian 侧同步 UI

注意：
- Obsidian 插件不直接写 Zotero
- Zotero 写操作仍通过 `zotero_companion_addon/` 在 Zotero 进程内执行
...
Invoke-RestMethod -Headers @{"X-Bridge-Token"=$token} -Uri "http://127.0.0.1:8765/health"
```

### 8.2 检查能力状态

```powershell
Invoke-RestMethod -Headers @{"X-Bridge-Token"=$token} -Uri "http://127.0.0.1:8765/capabilities"
```
...
如果你希望把 Obsidian 作为产品主入口，可以安装 `obsidian_bridge_plugin/`。

开发安装：

```powershell
.\scripts\build_obsidian_plugin.ps1 -VaultPluginsDir "E:\YourVault\.obsidian\plugins"
```

然后在 Obsidian 中启用：

```text
Settings -> Community plugins -> Zotero Agent Bridge
```

插件会在当前 vault 的插件目录下生成：

```text
.obsidian/plugins/zotero-agent-bridge/runtime/bridge-config.json
.obsidian/plugins/zotero-agent-bridge/runtime/bridge.log
```
...
当前 Obsidian 插件默认支持两种 bridge 启动方式：
- `Bridge executable path`：指向未来打包好的 bridge 可执行文件
- `Python executable path`：使用已安装 `zotero-agent-bridge` 的 Python 环境运行 `python -m zotero_agent_bridge`

正式产品化时，推荐把 Python bridge 打成平台可执行文件，再由 Obsidian 插件启动它，避免要求普通用户手动安装 Python。
